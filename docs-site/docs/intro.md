...
The current center of gravity is the complete `.pikelet` profile. A complete artifact contains the index, corpus rows, query interpretation metadata, and evaluation metadata behind a manifest identity. A reader can verify the manifest and segment hashes before serving results.

The docs site is intended to prove that model: every production build compiles these docs into a `.pikelet` file and mounts the browser search UI against that file.
