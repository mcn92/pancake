...
/**
 * DBpedia 50K Benchmark: Pancake Int8 vs Pancake FP32 vs hnswlib-node
 *
 * Uses the first 50K vectors from the DBpedia-OpenAI-1536D dataset with
 ...
 *   dbpedia_base_100k.fvecs   - 100K base vectors (float32, 1536D) — first 50K used
 *   dbpedia_query.fvecs       - query vectors
 *   dbpedia_groundtruth.ivecs - ground truth neighbor IDs
 *
 * Ground truth is recomputed for the 50K subset using brute-force L2,
 * cached to disk on first run. Pass --regenerate-gt to force recomputation.
 *
 * Usage:
 *   node benchmarks/benchmark_dbpedia_50k.js
 *   node benchmarks/benchmark_dbpedia_50k.js --regenerate-gt
 */
...
const LOG_PATH  = path.join(RESULTS_DIR, `dbpedia_50k_${timestamp}.log`);
const JSON_PATH = path.join(RESULTS_DIR, `dbpedia_50k_${timestamp}.json`);
const CSV_PATH  = path.join(RESULTS_DIR, `dbpedia_50k_${timestamp}.csv`);
...
// --- Ground truth (brute-force L2, cached to disk) ---
...
async function main() {
  ...
  log('='.repeat(70));
  log('DBpedia 50K Benchmark: Pancake Int8 vs FP32 vs hnswlib (L2)');
  log('='.repeat(70));
  ...
  // Ground truth (brute-force L2 on the 50K subset, cached)
  let groundTruth;
