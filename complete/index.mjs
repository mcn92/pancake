...
import { MAGIC, HEADER_BYTES, TABLE_ENTRY_BYTES, KIND_NAMES } from './format.mjs';
...
export async function openPancakeFile(input, options = {}) {
    ...
    try {
        ...
        if (qiKind === 1) {
            ...
            // segment data. Kernels are reader-owned (spec 3.6) and load
            // dynamically so kind-1/2 files never touch them; the node-only
            // glue is hidden from bundlers, the web glue stays analyzable so
            // browser builds carry it. options.createEncoder overrides the
            // loader for custom hosts.
            ...
            const bloomBytes = typeof Buffer !== 'undefined'
                ? Buffer.from(calibrationJson.vocabBloomBase64, 'base64')
                : Uint8Array.from(atob(calibrationJson.vocabBloomBase64), (c) => c.charCodeAt(0));
            const scorer3 = createAbstentionScorer(calibrationJson.asset, bloomBytes);
