...
// The complete-profile builder and reader live in pikelet-wasm
...
// True when this package runs from its checkout in the pikelet monorepo
// (REPO_ROOT holds the engine entry), where the in-tree pikelet-wasm must be
// preferred over any installed copy.
function inMonorepo() {
  return fssync.existsSync(path.join(REPO_ROOT, 'pikelet.node.mjs')) && fssync.existsSync(path.join(REPO_ROOT, 'complete', 'index.mjs'));
}
...
async function loadArtifactContract() {
  const fromRepo = () => import(pathToFileURL(path.join(REPO_ROOT, 'pikelet-artifact.js')).href);
...
async function loadPikelet() {
  const fromRepo = () => import(pathToFileURL(path.join(REPO_ROOT, 'pikelet.node.mjs')).href);
