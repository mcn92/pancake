...
The examples now tell one story: build search offline, ship it as application
data, run retrieval near the user, and hydrate results from the system that
...
encoder, docs metadata, and calibrated abstention. It has no storage binding and
no outbound query-time calls. The optional static Pages shell lives in
