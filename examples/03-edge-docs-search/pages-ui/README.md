...
private mode (`workers_dev = false` in `../wrangler.toml`), so its
`workers.dev` endpoint is disabled. Point the UI at your own deployment of the
Worker.
...
when the Worker is deployed with `PRIVATE_SEARCH=1` and a `DEMO_SEARCH_KEY`
secret.
